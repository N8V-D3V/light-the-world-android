...
## 1. Purpose
Describe how the approved Giving Machine presentation contract and protocol definitions will be translated into a focused Android presentation-slice structure for entry, expanded destination presentation, machine-window browsing, slot arming and explicit add confirmation, dispense presentation, contextual transitions, and accessibility interaction.
...
- `GivingMachineSelectionPresentationModule` - fulfills `GivingMachineSelectionProtocol` and owns visible slot arming, explicit add-to-cart confirmation presentation, brief dispense presentation state, and post-dispense armed-state clearing.
...
- `GivingMachineSelectionPresentationModule` depends on visible machine-window state, slot-selection requests, add-to-cart confirmation requests, current armed slot, and successful add-to-cart result inputs only.
...
3. Machine-window browse behavior:
   ...
   `GivingMachineWindowProtocol` returns either `machine window state` with visible numbered slots, window position, and continuation peeks, or `empty machine state` when the catalog is available but empty.
...
4. Armed-slot selection and explicit add confirmation:
   `machine window state`, `slot selection request`, and `current armed slot` enter `GivingMachineSelectionProtocol`.
   `GivingMachineSelectionProtocol` returns updated `slot selection state` and `add-to-cart confirmation state`, preserving the rule that a tapped visible slot becomes armed but is not added by itself.
...
- Protocol conformance tests:
  ...
  - Verify `GivingMachineWindowPresentationModule` returns a three-by-three numbered-slot machine window when enough items exist, correct continuation peeks for top, middle, bottom, and single-window states, and a non-transactional empty state when the catalog is available but empty.
  - Verify `GivingMachineSelectionPresentationModule` arms only visible slots, exposes separate confirmation state, rejects confirmation with no armed slot, and presents dispense or non-animated confirmation only after a successful upstream add result.
...
- Risk: slot arming and add confirmation could collapse into a single-tap add behavior.
  - Guardrail: keep `GivingMachineSelectionPresentationModule` authoritative for the armed-then-confirmed add flow and forbid add-to-cart presentation success without an explicit confirmation request.
